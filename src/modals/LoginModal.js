...
const LoginModal = ({visible, visible2, onHide, onSearch, onClick, onChange, 
    renderFooter, id, password, newPw, confPw, setId, setPassword, setNewPw, setConfPw}) => {
    return (
        <div>
             <div className="content-section implementation dialog-demo">
                <Dialog header="온라인 중계시스템 로그인" visible={visible} style={{width: '50vw'}} modal={true} footer={renderFooter('displayBasic')} onHide={() => onHide('displayBasic')}>
                    <div>
                ...
                <Dialog header="비밀번호 변경" visible={visible2} style={{width: '50vw'}} modal={true} footer={renderFooter('displayModal')} onHide={() => onHide('displayModal')}>  
                    <div>
                        <InputText id="email" className="loginId" placeholder="아이디 입력" type="text" value={id} onChange={(e) => setId(e.target.value)}/>
                    </div>
                    <div>
