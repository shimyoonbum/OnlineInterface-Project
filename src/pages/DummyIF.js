...
export class DummyIF extends Component {
    ...
    headerTemplate(data) {
        return <Button style={{float:'left'}} label={data.ifNm} icon="pi pi-plus" />;     //onClick={this.addNew}
        //return data.ifId;
    }
    ...
    actionStatus(rowData, column) {
        ...
        if(this.elemChecked && rowData.ifId===this.elemChecked.group && rowData.ifSeq!==this.elemChecked.id && isActive) {
            this.setState({
                [id]: false
            }); 
            isActive = false;
        ...
        return <ToggleButton id={id} checked={isActive} onLabel="활성" offLabel="비활성" onIcon="pi pi-check" offIcon="pi pi-times"
                onChange={(e) => {
                    this.setState({
                        [id]: e.value
                    }); 
    ...
    actionEdit(rowData, column) {
        ...
        return  <ToggleButton id={id} checked={true} onLabel="수정" onIcon="pi pi-pencil" offIcon="pi pi-times" 
                    onChange={(e) => {
   ...
   onSelect(e){
    ...
    setResJson() {
        ...
        try {
            ...
            this.messages.show({severity: 'error', summary: 'Error Message', detail: err.message});
            return resJson;
        }
    ...
    **/
    ...
    save() {
        ...
        this.commonService.setDummy(this.state.newDummy, this.state.dummy).then(data => {
            ...
            if(this.state.newDummy) {
                ...
                dummies[this.findSelectedIndex()] = this.state.dummy;

            this.setState({dummies:dummies, selectedSystem:null, newDummy: false, dummy: null, displayDialog:false});
    ...
    confirmDelete() {
        ...
        if(this.state.newDummy) {
            this.setState({selectedSystem:null, newDummy: false, dummy: null, displayDialog:false});
            return;
        }
    ...
    delete() {
        ...
        this.commonService.deleteDummy(this.state.dummy).then(data => {
            ...
            this.setState({dummies:dummies, selectedSystem:null, newDummy: false, dummy: null, visibleconfirmDelete:false, displayDialog:false});
    ...
    render() {
        ...
        const confirmDeleteFooter = (
            <div>
                ...
                <Button label="취소" icon="pi pi-times" onClick={() => this.setState({visibleconfirmDelete: false})} />
            </div>
        );
            ...
            <div>
                <div className="content-section introduction">
                    <div className="feature-intro">
                        ...
                        <p>각 인터페이스별 Dummy 응답을 추가/수정 및 활성화 시킬수 있습니다.</p>
                    </div>
                </div>

                <div className="content-section implementation">
                    <h3>조회할 시스템을 선택하세요!</h3>
                    <SelectButton value={this.state.selectedSystemId} options={this.state.systems} onChange={(e) => this.getDummies(e.value)} />
                    <br />

                    <DataTable header={header} //footer={footer} 
                        selectionMode="multi" selection={this.state.selectedDummy} 
                        onSelectionChange={e => this.setState({selectedDummy: e.value})} 
                        onRowSelect={this.onSelect}
                        value={this.state.dummies} rowGroupMode="subheader" sortField="ifNm" sortOrder={1} groupField="ifId"
                    ...
                    >
                        <Column body={this.actionStatus} style={{textAlign:'center', width: '10em'}}/>
                        <Column field="ifId" header="ID" />
                        <Column field="dummyNm" header="Dummy" />
                        <Column body={this.actionEdit} style={{textAlign:'center', width: '8em'}}/>
                    </DataTable>

                    <Dialog visible={this.state.displayDialog} style={{width: '500px'}} header="Dummy 인터페이스 상세" modal={true} footer={dialogFooter} onHide={() => this.setState({newDummy:false, displayDialog: false})} blockScroll={true}>
                        {
                            this.state.dummy &&
                            ...
                            <div className="p-grid p-fluid">
                                ...
                                {this.state.newDummy && 
                                <>
                                <div className="p-col-4" style={{padding:'.75em'}}><label htmlFor="ifId">ID</label></div>
                                <div className="p-col-8" style={{padding:'.5em'}}>
                                    <InputText id="ifId" onChange={(e) => {this.updateProperty('ifId', e.target.value)}} value={this.state.dummy.ifId}/>
                                </div>
                                </>
                                }

                                <div className="p-col-4" style={{padding:'.75em'}}><label htmlFor="dummyNm">Dummy</label></div>
                                <div className="p-col-8" style={{padding:'.5em'}}>
                                    <InputText id="dummyNm" onChange={(e) => {this.updateProperty('dummyNm', e.target.value)}} value={this.state.dummy.dummyNm}/>
                                </div>

                                <div className="p-col-4" style={{padding:'.75em'}}><label htmlFor="active">JSON</label></div>
                                <div className="p-col-8" style={{padding:'.5em'}}>
                                    <InputTextarea id="resJson" rows={15} cols={50} onChange={(e) => {this.updateProperty('resJson', e.target.value)}} value={this.setResJson()}/>
                                </div>

                                <Messages ref={(el) => this.messages = el} />

                                <Dialog header="삭제" footer={confirmDeleteFooter} visible={this.state.visibleconfirmDelete} style={{width: '25vw'}} modal={true} onHide={() => this.setState({visibleconfirmDelete: false})}>
