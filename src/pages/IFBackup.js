...
const IFBackup = () => {
    ...
    const renderFooter = name => {
        return (
            <div>
                <Button label="취소" icon="pi pi-times" onClick={() => onHide(name)} className="p-button-danger" />
                <Button label="복구" icon="pi pi-check" onClick={() => onRecovery()} className="p-button-text" autoFocus />
            </div>
        );
    }
    ...
    const getLogs = startRow => {
        if(tableNm.name == null)
            ...
            backUpService.getLogs(param).then(data => {                 
            ...
            <div className="content-section backup">
                <div className=".backup-intro">
                    ...
                    <div style={{padding : '10px 0px 0px 20px', textAlign : 'left'}}>
                        ...
                        <Button icon="pi pi-times" className="p-button-rounded p-button-danger p-button-text" style={{width:'27px', height:'27px'}} onClick={handleClick}/> 
                    ...
                    <div style={{display: 'flex', paddingTop : '10px'}}>
                        <div id="searchBtn" style={{textAlign : 'start', display : 'none', margin : '10px 10px 0px 10px', fontSize : '16px', float : 'left'}}>
                            <Button style={{width : '100px'}} label="검색" onClick={(e) => onSearch(e)} icon="pi pi-search" className="p-button"/>
                        </div>
                        <div id="recoverBtn" style={{textAlign : 'start', display : 'none', margin : '10px 10px 0px 10px', fontSize : '16px' , float : 'left'}}>
                            <Button style={{width : '100px'}} label="복구" onClick={() => onClick('displayConfirmation')} icon="pi pi-check" className="p-button"/>
                        </div>
                    </div>                       
            ...
            <div className="datatable-doc-demo">
                <div className="content-section implementation">
                    <DataTable 
                        ...
                        value={logs} scrollable scrollWidth="200px" responsive 
                        paginator={true} rows={rows} first={first} totalRecords={totalRecords} onPage={onPage} lazy={true}
                        ...
                        selection={selectedLogs} onSelectionChange={e => setSelectedLogs(e.value)} 
                    >
                        {dynamicColumns}
                    </DataTable>
                </div>
